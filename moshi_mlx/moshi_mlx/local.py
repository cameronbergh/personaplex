...
def server(printer_q, client_to_server, server_to_client, args):
    ...
    if args.text_prompt:
        log(f"[SERVER] Text prompt: {args.text_prompt}")
        text_prompt = wrap_with_system_tags(args.text_prompt)
        text_prompt_tokens = text_tokenizer.encode(text_prompt)
    ...
    if args.voice_prompt:
        log(f"[SERVER] Voice prompt: {args.voice_prompt}")
        # Need mimi to encode voice prompt
        ...
        # We need a tokenizer. Use rustymimi for simplicity as it is already a dependency
        # and we don't need gradients for encoding prompts.
        # Ideally we would use the same tokenizer as client.
        log(f"[SERVER] Loading mimi for voice prompt encoding from {mimi_file}")
        mimi = rustymimi.StreamTokenizer(mimi_file)

        gen.load_voice_prompt(args.voice_prompt)

        log("[SERVER] Stepping system prompts...")
        gen.step_system_prompts(mimi)
        log("[SERVER] System prompts done.")
